...
function speakThis(message) {
    ...
    else if(message.includes('100 + 100')) {
        const finalText = "é claro que é 200 você acha que eu não sei";
        speech.text = finalText;
    }

    else if(message.includes('claro que você não sabe')) {
        const finalText = "mas eu acabei de responder";
        speech.text = finalText;
    }
    ...
    else if(message.includes('o que é') || message.includes('who is') || message.includes('what are')) {
        window.open(`https://www.google.com/search?q=${message.replace(" ", "+")}`, "_blank");
        const finalText = "Isso é o que eu encontrei na internet sobre " + message;
        speech.text = finalText;
    }

    else if(message.includes('wikipedia')) {
        window.open(`https://en.wikipedia.org/wiki/${message.replace("wikipedia", "")}`, "_blank");
        const finalText = "Isso é o que eu encontrei na wikipedia sobre " + message;
        speech.text = finalText;
    }
    ...
    else if(message.includes('abrir editor de código')) {
        window.open("https://replit.com/~", "_blank");
        const finalText = "abrindo editor de código replit";
        speech.text = finalText;
    }

    else if(message.includes('toque marília mendonça')) {
        window.open("https://youtu.be/U2kVTHmCoB8", "_blank");
        const finalText = "abrindo";
        speech.text = finalText;
    }
